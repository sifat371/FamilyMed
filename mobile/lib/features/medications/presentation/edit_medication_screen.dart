...
class _EditMedicationScreenState extends ConsumerState<EditMedicationScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      if (!mounted) return;
      _nameController.text = medication.displayName;
      _strengthController.text = medication.strength ?? '';
      _dosageFormController.text = medication.dosageForm ?? '';
      _startDateController.text = _dateOnly(medication.startDate);
      _endDateController.text =
          medication.endDate == null ? '' : _dateOnly(medication.endDate!);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(l10n.editMedicine)),
      body: SafeArea(
        child: _loading
            ...
            : SingleChildScrollView(
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      TextFormField(
                        key: const Key('editMedicationName'),
                        controller: _nameController,
                        textInputAction: TextInputAction.next,
                        decoration:
                            InputDecoration(labelText: l10n.medicineName),
                        validator: (value) =>
                      ...
                      TextFormField(
                        key: const Key('editMedicationStrength'),
                        controller: _strengthController,
                        textInputAction: TextInputAction.next,
                        decoration:
                            InputDecoration(labelText: l10n.strengthLabel),
                      ),
                      const SizedBox(height: 16),
                      TextFormField(
                        key: const Key('editMedicationDosageForm'),
                        controller: _dosageFormController,
                        textInputAction: TextInputAction.next,
                        decoration:
                            InputDecoration(labelText: l10n.dosageForm),
                      ),
                      const SizedBox(height: 16),
                      TextFormField(
                        key: const Key('editMedicationStartDate'),
                      ...
                      const SizedBox(height: 16),
                      TextFormField(
                        key: const Key('editMedicationEndDate'),
